...
class PocketControllerTest {
    ...
    private RecurringTransactionDto recurringTransactionDto() {
        return new RecurringTransactionDto(1L, new BigDecimal("50.00"), Direction.EXPENSE,
                "Mensal", LocalDate.of(2025, 1, 1), null, "Academia");
    }
    ...
    void createRecurringTransaction_WhenDataIsValid_ShouldReturn201() throws Exception {
        ...
        mockMvc.perform(post("/pockets/1/recurring").with(user("test").authorities(List.of(new SimpleGrantedAuthority("USER"))))
                        ...
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.description").value("Academia"))
    ...
    void createRecurringTransaction_WhenUnauthenticated_ShouldReturn403() throws Exception {
        ...
        mockMvc.perform(post("/pockets/1/recurring")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    void findRecurringTransactions_ShouldReturn200WithList() throws Exception {
        when(pocketService.findRecurringTransactions(eq(1L), any()))
                .thenReturn(List.of(recurringTransactionDto()));

        mockMvc.perform(get("/pockets/1/recurring").with(user("test").authorities(List.of(new SimpleGrantedAuthority("USER")))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0].periodicity").value("Mensal"));
    }

    @Test
    void findRecurringTransactions_WhenUnauthenticated_ShouldReturn403() throws Exception {
        when(pocketService.findRecurringTransactions(eq(1L), any()))
                .thenReturn(List.of(recurringTransactionDto()));

        mockMvc.perform(get("/pockets/1/recurring"))
                .andExpect(status().isForbidden());
    }
    ...
    void deleteRecurringTransaction_WhenExists_ShouldReturn204() throws Exception {
        ...
        mockMvc.perform(delete("/pockets/1/recurring/1").with(user("test").authorities(List.of(new SimpleGrantedAuthority("USER")))))
                .andExpect(status().isNoContent());
    }

    @Test
    void deleteRecurringTransaction_WhenUnauthenticated_ShouldReturn403() throws Exception {
        doNothing().when(pocketService).deleteRecurringTransaction(eq(1L), eq(1L), any());

        mockMvc.perform(delete("/pockets/1/recurring/1"))
                .andExpect(status().isForbidden());
    }

    @Test
    void deleteRecurringTransaction_WhenNotFound_ShouldReturn404() throws Exception {
        doThrow(new EntityNotFoundException("Recurring transaction not found"))
                .when(pocketService).deleteRecurringTransaction(eq(1L), eq(99L), any());

        mockMvc.perform(delete("/pockets/1/recurring/99").with(user("test").authorities(List.of(new SimpleGrantedAuthority("USER")))))
                .andExpect(status().isNotFound());
    }
}